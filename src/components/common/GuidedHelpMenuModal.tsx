...
export const GuidedHelpMenuModal: React.FC = () => {
  ...
  if (isPhotoEnhancerOpen) {
    ...
    if (sellerTab === 'COLLABORATE' || sellerTab === 'MESSAGES') {
      currentTourId = 'SELLER_COLLABORATION_TOUR';
    } else {
  ...
  const handleLaunchCollaborationTour = () => {
    setIsHelpMenuOpen(false);
    if (!guidedHelpEnabled) {
      setGuidedHelpEnabled(true);
    }
    setSellerTab('COLLABORATE');
    startTour('SELLER_COLLABORATION_TOUR', 0);
  };
      ...
      >
        ...
        <div className="p-3 rounded-xl bg-surface-container-low border border-outline/20 flex items-center justify-between text-xs">
          ...
          <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-primary/10 text-primary">
            {activeMode === 'ARTISAN' ? t('Artisan Studio') : t('Customer Mode')}
          </span>
        </div>
        ...
        <div className="space-y-2">
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-[11px] text-on-surface-variant">
                {t('Highlighted walkthrough of key actions and elements.')}
              </p>
            </div>
          </button>
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-[11px] text-on-surface-variant">
                {t('Hear instructions spoken aloud in your selected language.')}
              </p>
            </div>
          </button>
          ...
          {activeMode === 'CUSTOMER' && (
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-[11px] text-on-surface-variant">
                  {t('Learn how to explore state GI clusters and crafts.')}
                </p>
              </div>
            </button>
          )}

          {/* 3B. Seller Collaboration Tour (Artisan Mode) */}
          {activeMode === 'ARTISAN' && (
            <button
              onClick={handleLaunchCollaborationTour}
              className="w-full p-3 rounded-xl border border-outline/20 hover:border-amber-500/40 hover:bg-amber-500/5 transition text-left flex items-center gap-3.5 cursor-pointer group"
            >
              <div className="w-8 h-8 rounded-lg bg-amber-500/15 text-amber-700 dark:text-amber-300 flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
              ...
              <div className="flex-1">
                <h4 className="text-xs font-bold text-on-surface">
                  {TOURS.SELLER_COLLABORATION_TOUR.name[language] || 'Artisan Collaboration Tour'}
                </h4>
                <p className="text-[11px] text-on-surface-variant">
                  {t('Partner with master creators and build joint collections.')}
                </p>
              </div>
            </button>
          )}
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-[11px] text-on-surface-variant">
                {t('Reset progress and replay step-by-step guidance from Step 1.')}
              </p>
            </div>
          </button>
        </div>
        ...
        <div className="p-3.5 rounded-xl bg-surface-container-low border border-outline/20 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-on-surface">{t('Guided Help')}</span>
            <button

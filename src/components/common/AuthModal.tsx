          ...
          <div className="relative z-10 space-y-6">
            <div className="flex items-center gap-2">
              ...
              <span className="font-serif text-xl font-bold text-on-surface">
                Desi<span className="text-primary italic font-normal">{t('Craft')}</span>
              </span>
            </div>
            ...
            <div className="space-y-2">
              <span className="inline-block px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-secondary/15 text-secondary border border-secondary/30">
                {t('BHARAT LIVING HERITAGE')}
              </span>
              <h2 className="font-serif text-2xl lg:text-3xl font-bold text-on-surface leading-snug">
                {t('India’s crafts are more than products.')}
              </h2>
              <p className="text-xs text-on-surface-variant leading-relaxed">
                {t('They carry ancestral stories, mathematical memories, and centuries of living lineage. Discover the master artisans behind them.')}
              </p>
            </div>
            ...
            <div className="space-y-3 pt-2">
              <div className="flex items-center gap-2.5 text-xs text-on-surface font-medium">
                ...
                <span>{t('Certified Geographical Indication (GI) Crafts')}</span>
              </div>
              ...
              <div className="flex items-center gap-2.5 text-xs text-on-surface font-medium">
                ...
                <span>{t('National Master Artisans & Weaving Guilds')}</span>
              </div>
              ...
              <div className="flex items-center gap-2.5 text-xs text-on-surface font-medium">
                ...
                <span>{t('Oral Heritage Stories in 10 Mother Tongues')}</span>
              </div>
              ...
              <div className="flex items-center gap-2.5 text-xs text-on-surface font-medium">
                ...
                <span>{t('Cryptographic Digital Craft Passports')}</span>
              </div>
            </div>
          </div>
          ...
          <div>
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between text-xs">
                      <label className="font-semibold text-on-surface flex items-center gap-1">
                        ...
                        <span>{contactType === 'PHONE' ? t('Phone Number') : t('Email Address')} *</span>
                      </label>
                      <button
                        type="button"
                        onClick={() => {
                      ...
                      >
                        {t('Use')} {contactType === 'PHONE' ? t('Email instead') : t('Phone instead')}
                      </button>
                    </div>
                      ...
                      {password && (
                        <div className="space-y-1 pt-0.5">
                          ...
                          <div className="flex justify-between text-[10px] text-on-surface-variant">
                            <span>{t('Strength:')} {strength.label}</span>
                            <span>{t('Min. 8 characters')}</span>
                          </div>
                        </div>
                      )}
                    </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <span className="text-xs font-serif font-bold text-on-surface">
                            {language.toUpperCase()} ({t('Selected')})
                          </span>
                        </div>
                      </div>
                      <button
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div className="space-y-1">
                        ...
                        <input
                          type="text"
                          required
                          placeholder={t('e.g. Varanasi, Hyderabad')}
                          value={cityLocation}
                    ...
                    <div className="flex items-start gap-2 pt-1">
                      ...
                      <label htmlFor="terms-check" className="text-[11px] text-on-surface-variant leading-tight cursor-pointer">
                        {t('I agree to the')} <strong className="text-on-surface">{t('Terms of Service')}</strong>, <strong className="text-on-surface">{t('Privacy Policy')}</strong>, {t('and the')} <strong className="text-primary">{t('Indian Handcraft Code of Ethics')}</strong>.
                      </label>
                    </div>
                    ...
                    >
                      <span>{t('Create Universal Account')}</span>
            ...
            {activeTab === 'LOGIN' && (
              <form onSubmit={handleLoginSubmit} className="space-y-4">
                <div>
                  <h3 className="font-serif text-xl font-bold text-on-surface">
                    {t('Welcome back to Desi Craft')}
                  </h3>
                  <p className="text-xs text-on-surface-variant mt-0.5">
                    {t('Sign in to your universal account to manage loom orders or patronize heirlooms.')}
                  </p>
                </div>
                ...
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-on-surface flex items-center gap-1">
                    <UserIcon className="w-3.5 h-3.5 text-primary" />
                    <span>{t('Phone Number or Email')} *</span>
                  </label>
                  <input
                    type="text"
                    required
                ...
                <button
                  ...
                  className="w-full py-3 rounded-full bg-primary text-on-primary text-xs sm:text-sm font-bold hover:bg-primary/90 transition shadow-md flex items-center justify-center gap-2 cursor-pointer mt-2"
                >
                  <span>{t('Sign In to Desi Craft')}</span>
                  <ArrowRight className="w-4 h-4" />
                </button>
              </form>
            )}

      ...
      <div className="p-6 sm:p-8 rounded-2xl bg-surface-container-low border border-outline/20">
        <div className="max-w-2xl space-y-3">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-secondary/15 text-secondary border border-secondary/30">
            ...
            <span>{t('GOVERNMENT SCHEME TRACKER')}</span>
          </div>
          <h2 className="font-serif text-2xl sm:text-3xl font-bold text-on-surface">
            {t('PM Vishwakarma Scheme Navigator')}
          </h2>
          <p className="text-xs sm:text-sm text-on-surface-variant leading-relaxed">
            {t('Track benefits, biometric verification, subsidized toolkits, and institutional credit.')}
          </p>
        </div>
      </div>
      ...
      <div className="p-6 rounded-2xl bg-surface border border-outline/20 space-y-5 shadow-xs">
        <div className="flex items-center justify-between">
          <h3 className="font-serif text-base font-bold text-on-surface flex items-center gap-2">
            ...
            <span>{t('Your Verification Progress')}</span>
          </h3>
          <span className="text-xs font-bold text-primary font-mono">
            {completedSteps.length} {t('of')} 5 {t('Milestones Active')}
          </span>
        </div>
        ...
        <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
          <span className="text-on-surface-variant">
            {t('Official Ministry Portal:')} <strong>pmvishwakarma.gov.in</strong>
          </span>
          <a
          ...
          >
            <span>{t('Apply on National Portal')}</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-surface border border-outline/20 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-semibold text-primary">{t('Invest India')}</span>
          </div>
          <h4 className="font-serif text-base font-bold text-on-surface">
            {t('One District One Product (ODOP) Display Stalls')}
          </h4>
          <p className="text-xs text-on-surface-variant leading-relaxed">
            {t('Registered master artisans can claim free retail stalls at major international airports, railway terminals, and embassies globally.')}
          </p>
          <a
          ...
          >
            <span>{t('Check District Product Catalog')}</span>
        ...
        <div className="p-5 rounded-2xl bg-surface border border-outline/20 space-y-3">
          <div className="flex items-center justify-between">
            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-primary/10 text-primary">
              {t('MINISTRY OF TEXTILES')}
            </span>
            <span className="text-xs font-semibold text-primary">{t('DC Handicrafts')}</span>
          </div>
          <h4 className="font-serif text-base font-bold text-on-surface">
            {t('Pehchan National Artisan Identity Card')}
          </h4>
          <p className="text-xs text-on-surface-variant leading-relaxed">
            {t('Universal photo identity card granting free health insurance under Ayushman Bharat and direct stall allotment at Dilli Haat and Surajkund Mela.')}
          </p>
          <a
            ...
            className="text-xs font-bold text-primary hover:underline flex items-center gap-1"
          >
            <span>{t('Register for Pehchan Card')}</span>
            <ArrowRight className="w-3 h-3" />
          </a>
        </div>
      </div>

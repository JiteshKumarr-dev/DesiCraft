...
export const FairPriceAdvisorModal: React.FC = () => {
  const { isPriceAdvisorOpen, setIsPriceAdvisorOpen } = useApp();
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-on-surface-variant">
                Calculate equitable market value ensuring dignity, living wages, and heritage preservation.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4 p-4 rounded-xl bg-surface-container-low border border-outline/20">
            <div className="space-y-1.5">
              <div className="flex justify-between text-xs">
                <span className="font-semibold text-on-surface">Indigenous Raw Material Cost</span>
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between text-xs">
                <span className="font-semibold text-on-surface">Artisan Labor Time on Traditional Loom</span>
                <span className="font-bold text-primary font-mono">{laborDays} Days</span>
              </div>
              <input
                type="range"
                min="1"
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-on-surface block">Craft Mastery Level</label>
              <div className="grid grid-cols-3 gap-2">
                {[
                  { id: 'MODERATE', label: 'Skilled Artisan' },
                  { id: 'HIGH', label: 'Senior Master' },
                  { id: 'MASTER', label: 'National Awardee' },
                ].map((c) => (
                  <button
                    key={c.id}
                    type="button"
          ...
          <div className="p-5 rounded-2xl bg-surface-container border-2 border-primary/30 space-y-4">
            <div className="flex items-baseline justify-between border-b border-outline/10 pb-3">
              ...
              <span className="px-3 py-1 rounded-full text-xs font-bold bg-green-100 text-green-800 flex items-center gap-1">
                ...
                {breakdown.artisan_direct_share_percent}% To Artisan Family
              </span>
            </div>

            <div className="space-y-2 text-xs">
              <div className="flex justify-between text-on-surface-variant">
                <span>Direct Artisan Labor ({laborDays} days @ ₹{breakdown.daily_fair_wage}/day living wage):</span>
                <span className="font-semibold text-on-surface">₹{breakdown.total_labor_cost.toLocaleString('en-IN')}</span>
              </div>
              <div className="flex justify-between text-on-surface-variant">
                <span>Pure Natural Materials:</span>
                <span className="font-semibold text-on-surface">₹{breakdown.raw_material_cost.toLocaleString('en-IN')}</span>
              </div>
              <div className="flex justify-between text-on-surface-variant">
                <span>GI Provenance & Intangible Cultural Lineage:</span>
            ...
            <div className="p-3 rounded-xl bg-surface-container-low border border-outline/10 text-xs space-y-1">
              <span className="text-[11px] font-bold text-red-600 block">
                Typical Commercial Middleman Exploitation:
              </span>
